...
func _advance_preview(delta: float) -> void:
	...
	elapsed += delta
	var rep_duration: float = maxf(current_rep_duration, 0.001)
	var rep_time: float = fmod(elapsed, rep_duration)
	var cycle: float = rep_time / rep_duration
	current_rep_index = int(floor(elapsed / rep_duration)) % 3

	var frame_phase: float = clampf(cycle, 0.0, 1.0)
	var frame_index: int = int(round(frame_phase * float(current_frame_count - 1)))
	var frame_motion: float = float(frame_index) / float(maxi(current_frame_count - 1, 1))
	motion_rom = frame_motion if frame_motion <= 0.5 else 1.0 - frame_motion
	motion_rom = clampf(motion_rom * 2.0, 0.02, 1.0)
	forward = cycle < 0.5

func _apply_visuals() -> void:
	if current_frames.size() > 0 and athlete_frame:
		var frame_index: int = int(round(motion_rom * float(current_frames.size() - 1)))
	...
	if rom_overlay and rom_overlay.has_method("set_motion"):
		var zone: Dictionary = _get_speed_zone(current_speed_profile, motion_rom)
		rom_overlay.call("set_motion", motion_rom, float(zone.get("start", 0.0)), float(zone.get("end", 1.0)), forward)

	if rom_marker and rom_marker.has_method("set_in_zone"):
		var zone: Dictionary = _get_speed_zone(current_speed_profile, motion_rom)
		var zone_start: float = float(zone.get("start", 0.0))
		var zone_end: float = float(zone.get("end", 1.0))
		rom_marker.call("set_in_zone", motion_rom >= zone_start and motion_rom <= zone_end)
...
func _find_speed_profile(profile_key: String) -> Dictionary:
	var speed_range: Array = (config.get("standardCableSpeedRange", {}) as Dictionary).get("exercises", [])
...
func _get_speed_zone(standard: Dictionary, cycle: float) -> Dictionary:
	...
	var forward_phase: bool = cycle < 0.5
	var half_phase: float = cycle * 2.0 if forward_phase else (cycle - 0.5) * 2.0
	var speed_window: Dictionary = standard.get("pullOutSpeed", {}) if forward_phase else standard.get("returnInSpeed", {})
	var min_speed: float = float(speed_window.get("min", 0.0))
	var max_speed: float = float(speed_window.get("max", 0.0))
	var avg_speed: float = (min_speed + max_speed) * 0.5
	if avg_speed <= 0.0:
		return {"start": 0.0, "end": 1.0}

	var min_progress: float = (min_speed / avg_speed) * half_phase
	var max_progress: float = (max_speed / avg_speed) * half_phase
	var start: float = minf(min_progress, max_progress)
	var end: float = maxf(min_progress, max_progress)
	var center: float = half_phase
	...
	return {
		"start": clampf(start, 0.0, 1.0 - min_span),
		"end": clampf(end, min_span, 1.0)
	}
